...
//back to store
app.put('/backToStore/:id', (req, res) => {
    const sql = "UPDATE transfer set `date_expected` = ? where id = ?";
    const values = [
        ''
    ]
    const id = req.params.id;
...
app.get("/getProductCustodians/:id", (req, res) => {
    const sql = `SELECT transfer.id id, product_id, custodian_id, location_id, quantity, date_moved, date_expected, c.name as cname, l.name as lname, u.name as tunit FROM transfer, custodian c, location l, unit u WHERE product_id = ? and c.id = transfer.custodian_id and l.id = transfer.location_id and u.id = transfer.unit_id and transfer.date_expected != '' `;
    const id = req.params.id;
    db.query(sql, [id], (err, data) => {
        if (err) {
            console.error(err);
...
app.get("/post_date", (req, res) => {
    const sql = `SELECT * FROM transfer_details where date_expected != '' date_expected < CURRENT_DATE`;
    db.query(sql, (err, data) => {
        if(err) return res.json("Error");
        return res.json(data);
    })
