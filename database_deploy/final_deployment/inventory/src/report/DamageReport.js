...
function DamageReport() {
  ...
  const fetchData = () => {
    axios.get(`https://ghaacademy.com.ng/getDamagedItems`)
  ...
  return (
    <div className='main-content' id='printable-content'>
      <Container>
      <div id="non-print">
        <h5 className='mt-4'>Damage Report</h5>
        <div className='row'>
          ...
          <tbody>

            {
              sortedInventory.filter((data) => {
                ...
                <tr key={i}>
                <NavLink to={`/supply/manageSupply/${data.product_id}`} >
                      {data.product}
                    </NavLink>
                <td>{data.unit}</td>
                <td>{data.quantity}</td>
