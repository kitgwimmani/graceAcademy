...
function LocationReport() {
  ...
  useEffect(() => {
    axios.get('https://ghaacademy.com.ng/location').then(res => setAllLocations(res.data))
      .catch(err => console.log(err));
  }, [])
    ...
    <div className='main-content' id='printable-content'>
      ...
      <div id="non-print">
        ...
        <Table striped bordered hover style={{ fontSize: '12px' }} ref={tableRef}>
          <thead>
            <tr>
              <th onClick={() => handleSort('date_moved')}><FaSort /> Date Moved</th>
              <th onClick={() => handleSort('location')}><FaSort /> Location</th>
              <th onClick={() => handleSort('product')}><FaSort /> Item</th>
              <th onClick={() => handleSort('custodian')}><FaSort /> Custodian</th>
